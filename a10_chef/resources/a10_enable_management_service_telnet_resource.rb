resource_name :a10_enable_management_service_telnet
...
action :create do
    ...
    get_url = "/axapi/v3/enable-management/service/telnet"
    ...
    params = { "telnet": {"tunnel-cfg": tunnel_cfg,
    ...
    params[:"telnet"].each do |k, v|
        if not v 
            params[:"telnet"].delete(k)
        end
    end
    ...
    begin
        ...
        converge_by('Creating telnet') do
...
action :update do
    client = new_resource.client
    a10_name = new_resource.a10_name
    url = "/axapi/v3/enable-management/service/telnet"
    tunnel_cfg = new_resource.tunnel_cfg
    management = new_resource.management
    uuid = new_resource.uuid
    acl_v6_list = new_resource.acl_v6_list
    ...
    found_diff = false 
    result["telnet"].each do |k, v|
        if v != params[:"telnet"][k] 
            found_diff = true
        end
    end

    if found_diff
        converge_by('Updating telnet') do
...
action :delete do
    ...
    if result
        converge_by('Deleting telnet') do
            client.delete(url)
        end
    end
end