resource_name :a10_snmp_server_community_read_oid
...
action :create do
    ...
    post_url = "/axapi/v3/snmp-server/community/read/%<user>s/oid/"
    get_url = "/axapi/v3/snmp-server/community/read/%<user>s/oid/%<oid-val>s"
...
action :update do
    client = new_resource.client
    a10_name = new_resource.a10_name
    url = "/axapi/v3/snmp-server/community/read/%<user>s/oid/%<oid-val>s"
    remote = new_resource.remote
    oid_val = new_resource.oid_val
    user_tag = new_resource.user_tag
    uuid = new_resource.uuid
