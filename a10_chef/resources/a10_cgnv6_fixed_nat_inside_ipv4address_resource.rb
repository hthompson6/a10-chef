resource_name :a10_cgnv6_fixed_nat_inside_ipv4address

property :a10_name, String, name_property: true
property :partition, String,required: true
property :inside_netmask, String,required: true
...
action :create do
    ...
    post_url = "/axapi/v3/cgnv6/fixed-nat/inside/ipv4address/"
    get_url = "/axapi/v3/cgnv6/fixed-nat/inside/ipv4address/%<inside-start-address>s+%<inside-end-address>s+%<inside-netmask>s+%<partition>s"
    ...
    params = { "ipv4address": {"partition": partition,
    ...
    params[:"ipv4address"].each do |k, v|
        if not v 
            params[:"ipv4address"].delete(k)
        end
    end
    ...
    begin
        ...
        converge_by('Creating ipv4address') do
...
action :update do
    client = new_resource.client
    a10_name = new_resource.a10_name
    url = "/axapi/v3/cgnv6/fixed-nat/inside/ipv4address/%<inside-start-address>s+%<inside-end-address>s+%<inside-netmask>s+%<partition>s"
    partition = new_resource.partition
    inside_netmask = new_resource.inside_netmask
    uuid = new_resource.uuid
    nat_end_address = new_resource.nat_end_address
    ...
    found_diff = false 
    result["ipv4address"].each do |k, v|
        if v != params[:"ipv4address"][k] 
            found_diff = true
        end
    end

    if found_diff
        converge_by('Updating ipv4address') do
...
action :delete do
    ...
    if result
        converge_by('Deleting ipv4address') do
            client.delete(url)
        end
    end
end