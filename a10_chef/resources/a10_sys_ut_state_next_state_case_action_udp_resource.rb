resource_name :a10_sys_ut_state_next_state_case_action_udp
...
action :create do
    ...
    get_url = "/axapi/v3/sys-ut/state/%<name>s/next-state/%<name>s/case/%<case-number>s/action/%<direction>s/udp"
...
action :update do
    client = new_resource.client
    a10_name = new_resource.a10_name
    url = "/axapi/v3/sys-ut/state/%<name>s/next-state/%<name>s/case/%<case-number>s/action/%<direction>s/udp"
    uuid = new_resource.uuid
    checksum = new_resource.checksum
    nat_pool = new_resource.nat_pool
    src_port = new_resource.src_port
