resource_name :a10_slb_virtual_server_port_stats_http_vport
...
action :create do
    ...
    get_url = "/axapi/v3/slb/virtual-server/%<name>s/port/%<port-number>s+%<protocol>s/stats?http_vport=true"
...
action :update do
    client = new_resource.client
    a10_name = new_resource.a10_name
    url = "/axapi/v3/slb/virtual-server/%<name>s/port/%<port-number>s+%<protocol>s/stats?http_vport=true"
    stats = new_resource.stats
