resource_name :a10_backup_log_old
...
action :create do
    ...
    get_url = "/axapi/v3/backup/log-old"
    ...
    params[:"log-old"].each do |k, v|
        if not v 
            params[:"log-old"].delete(k)
        end
    end
    ...
    begin
        ...
        converge_by('Creating log-old') do
...
action :update do
    ...
    found_diff = false 
    result["log-old"].each do |k, v|
        if v != params[:"log-old"][k] 
            found_diff = true
        end
    end

    if found_diff
        converge_by('Updating log-old') do
...
action :delete do
    ...
    if result
        converge_by('Deleting log-old') do
            client.delete(url)
        end
    end
end